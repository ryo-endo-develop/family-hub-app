...
開発中の動作確認や手動テストを容易にするために、データベースの状態をリセットしたり、初期データ（シードデータ）を投入したりするスクリプトを用意しています。
コマンドはプロジェクトルート（`compose.yml` があるディレクトリ）から実行してください。

- **データベースのリセット＆初期データ投入:**
  現在の全てのデータを削除し、テーブルを再作成した後、定義済みの初期データを投入します。

  ```bash
  docker compose run --rm backend python scripts/seed_data.py --reset
  ```

  - `--reset`: このオプションを付けると、シードデータ投入前に DB リセットが実行されます。

- **初期データの投入のみ (リセットなし):**
  現在のデータベース状態に、定義済みの初期データを追加します。（通常はリセットしてから実行します）
...
- **初期データの内容:**
  スクリプト (`scripts/seed_data.py`) 内の `seed_initial_data` 関数で定義されています。現在はテスト用の家族、ユーザー、ラベル、タスク（定常、単発、サブタスク含む）が含まれています。必要に応じてこの関数を編集してください。
